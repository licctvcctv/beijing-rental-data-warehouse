...
SELECT
    name,
    level,
    CASE WHEN region = '\\N' THEN '未知' ELSE region END AS region_std,
    address,
    price,
...
SELECT
    ...
    show_time,
    venue,
    CASE WHEN region = '北京市' OR region = '\\N' THEN '北京市' ELSE region END AS region_std,
    price_range,
    COALESCE(price_min, CAST(0 AS DECIMAL(10,2))) AS price_min,
    COALESCE(price_max, COALESCE(price_min, CAST(0 AS DECIMAL(10,2)))) AS price_max,
    CASE
        WHEN status IN ('售票中', '预售中', '已结束') THEN status
        WHEN status = '\\N' THEN '待定'
        ELSE status
    END AS status_std,
...
SELECT
    name,
    CASE WHEN venue_type = '\\N' THEN '其他场馆' ELSE venue_type END AS venue_type_std,
    CASE WHEN region = '\\N' THEN '未知' ELSE region END AS region_std,
    address,
    COALESCE(score, CAST(0 AS DECIMAL(10,2))) AS score_num,
